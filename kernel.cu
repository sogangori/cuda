...
__global__ void filter_1d_kernel(float* dst, float*src, float*filter, int length, int m, int radius){
	...
	if (index < out_length){//유효한 스레드만 작업하기 
		...
		int t = index%dst_length;//t:0~3999
...
__host__ void filter_1d_device(float* dst, float*src, float*filter, int length, int m, int radius){
	...
	int M = 32;//스레드 수
	int N = out_length;
	filter_1d_kernel<<<(N+M-1)/M,M>>>(dst,src, filter, length, m, radius);
	clock_t end = clock();
...
void filter_1d(){
	int length = 4000;//샘플링된 signal 4천개
	int m = 128;// 128회 받음	
	int filter_length = 15;//필터가 클수록 공유메모리 사용하는 것이 이득
